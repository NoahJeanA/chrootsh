...
//go:embed configs/bashrc.template
var bashrcTemplate string

func setupHome(chrootDir, homeDir string, uid int) error {
	fullHome := chrootDir + homeDir
	os.MkdirAll(fullHome, 0755)
	os.Chown(fullHome, uid, uid)

	// Create shell configuration
	bashrc := fullHome + "/.bashrc"
	bashrcContent, err := getBashrcContent()
	if err != nil {
		return fmt.Errorf("failed to generate bashrc: %w", err)
	}
	os.WriteFile(bashrc, []byte(bashrcContent), 0644)
	os.Chown(bashrc, uid, uid)

	bashProfile := fullHome + "/.bash_profile"
	bashProfileContent := `# Bash Profile
if [ -f ~/.bashrc ]; then
    . ~/.bashrc
fi
`
	os.WriteFile(bashProfile, []byte(bashProfileContent), 0644)
	os.Chown(bashProfile, uid, uid)

	inputrc := fullHome + "/.inputrc"
	inputrcContent := `set editing-mode emacs
set bell-style none
`
	os.WriteFile(inputrc, []byte(inputrcContent), 0644)
	os.Chown(inputrc, uid, uid)

	return nil
}

func getBashrcContent() (string, error) {
	tmpl, err := template.New("bashrc").Parse(bashrcTemplate)
	if err != nil {
		return "", fmt.Errorf("failed to parse bashrc template: %w", err)
	}
	...
	if err := tmpl.Execute(&buf, data); err != nil {
		return "", fmt.Errorf("failed to execute bashrc template: %w", err)
	}
	
	return buf.String(), nil
}

func getBashrcContentOLD() string {
